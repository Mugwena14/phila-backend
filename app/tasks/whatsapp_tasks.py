...
def send_appointment_reminder(self, booking_id: str, hours_before: int):
    """
    Sends a WhatsApp reminder + in-app notification X hours before appointment.
    ...
    try:
        ...
        success = send_whatsapp_message(patient.phone, whatsapp_msg)
        if not success:
        ...
        if hours_before == 1 or hours_before == 2:
        ...
        logger.info(f"Reminder sent ({hours_before}hr) for booking {booking_id}")
...
def check_intake_completion(self, booking_id: str):
    ...
    If patient hasn't completed intake, bump risk score +20 and nudge.
    """
    import redis
    import json
    from app.core.config import settings as app_settings
