...
@router.get("/{doctor_id}/favorite-status")
def get_favorite_status(
    doctor_id: UUID,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Tells the profile screen whether THIS patient has already
    favorited THIS doctor, so the heart icon renders correctly on
    load rather than always starting unfilled."""
    is_favorited = db.query(FavoriteDoctor).filter(
