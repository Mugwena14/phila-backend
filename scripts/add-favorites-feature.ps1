...
@router.get("/favorites/me", response_model=List[DoctorResponse])
def get_my_favorite_doctors(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Account-synced favorites list - works across devices, tied to
    the patient's login, not local device storage."""
    return (
        db.query(Doctor)
        .join(FavoriteDoctor, FavoriteDoctor.doctor_id == Doctor.id)
        .filter(FavoriteDoctor.patient_id == current_user.id)
...
@router.post("/{doctor_id}/favorite", response_model=FavoriteToggleResponse)
def favorite_doctor(
    doctor_id: UUID,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
    ...
    if not existing:
        db.add(FavoriteDoctor(patient_id=current_user.id, doctor_id=doctor_id))
        db.commit()

    return FavoriteToggleResponse(favorited=True, doctor_id=doctor_id)


@router.delete("/{doctor_id}/favorite", response_model=FavoriteToggleResponse)
def unfavorite_doctor(
    doctor_id: UUID,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    # Idempotent - calling this on something already unfavorited is not
    # an error, same philosophy as elsewhere in this codebase (offline
    # queue retries, etc.) - avoids spurious failures on double-taps.
    db.query(FavoriteDoctor).filter(
        FavoriteDoctor.patient_id == current_user.id,
        FavoriteDoctor.doctor_id == doctor_id,
    ).delete()
    db.commit()

    return FavoriteToggleResponse(favorited=False, doctor_id=doctor_id)
